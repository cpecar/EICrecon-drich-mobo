...
void richgeo::IrtGeoDRICH::DD4hep_to_IRT() {
  ...
  for (int isec = 0; isec < nSectors; isec++) {
    ...
    dd4hep::Position mirrorCenter_1(
      ...
      m_det->constant<double>("DRICH_mirror_1_center_z_"+secName) / dd4hep::mm
				    );
    dd4hep::Position mirrorCenter_2(
      m_det->constant<double>("DRICH_mirror_2_center_x_"+secName) / dd4hep::mm,
    ...
    auto mirrorHS1_point_z_1 = m_det->constant<double>("DRICH_mirror_1_halfspace1_point_z_"+secName) / dd4hep::mm;

    auto mirrorHS1_point_x_2 = m_det->constant<double>("DRICH_mirror_2_halfspace1_point_x_"+secName) / dd4hep::mm;
    ...
    auto mirrorHS1_norm_z_1 = m_det->constant<double>("DRICH_mirror_1_halfspace1_dir_z_"+secName);

    auto mirrorHS1_norm_x_2 = m_det->constant<double>("DRICH_mirror_2_halfspace1_dir_x_"+secName);
    ...
    m_mirrorOpticalBoundary_1  = new OpticalBoundary(
        ...
        m_mirrorSphericalSurface_1,            // surface
        false,                                // bool refractive
	true                                 // bool reflective
        );
    m_mirrorOpticalBoundary_2  = new OpticalBoundary(
        m_irtDetector->GetContainerVolume(), // CherenkovRadiator radiator
        m_mirrorSphericalSurface_2,            // surface
        false,                                // bool refractive
	true                                 // bool reflective
        );
    ...
    m_log->trace("    mirror x = {:f} mm", mirrorCenter_1.x());
    m_log->trace("    mirror y = {:f} mm", mirrorCenter_1.y());
    m_log->trace("    mirror z = {:f} mm", mirrorCenter_1.z());
    m_log->trace("    mirror R = {:f} mm", mirrorRadius_1);

    m_log->trace("");
    m_log->trace("  SECTOR {:d} MIRROR 2:", isec);
    m_log->trace("    mirror x = {:f} mm", mirrorCenter_2.x());
    m_log->trace("    mirror y = {:f} mm", mirrorCenter_2.y());
    ...
    m_mirrorBackplaneSurface = new FlatSurface(TVector3(0, 0, maxZs[minIndex]), normX, normY);
    
    auto rad = m_irtDetector->GetRadiator(RadiatorName(kGas).c_str());    
    if(rad){
      rad->m_Borders[isec].second = static_cast<ParametricSurface*>(m_mirrorBackplaneSurface); // TODO: which mirror/how do we define this boundary? mirror with smallest R+z?
    }
...
richgeo::IrtGeoDRICH::~IrtGeoDRICH() {
  ...
  delete m_mirrorSphericalSurface_1;
  delete m_mirrorBackplaneSurface;
